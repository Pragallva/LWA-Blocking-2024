...
if __name__ == "__main__":
        ...
        FILTER_DIREC = "band_pass_time_filter_004-infy_days"
        route     = '/data/pragallva/2023_repeat_ERA5/post_processing/filtered_parameters/%s/'%(FILTER_DIREC)        
        dest = route
        ######################################################################################################
        ######## WAVE ACTIVITY BUDGET TERMS and THE WAVE ACTIVITY FLUXES ARE SAVED IN SMOOTHENED TERMS #######
        ######################################################################################################
        
        file_check = route+'/EPa_and_advective_fluxes.hdf5'
        ...
        if not os.path.exists(file_check):
                ...
                logging_object.write('-----------------------------------------------------------------------------------')
                logging_object.write('CROSS CORRELATION TERMS ARE CALCULATED, GOOD JOB !!!')
                logging_object.write('#####################################################################################')
