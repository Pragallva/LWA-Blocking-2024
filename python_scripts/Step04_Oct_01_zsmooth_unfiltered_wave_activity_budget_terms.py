...
if __name__ == "__main__":
        ...
        FILTER_DIREC = "unfiltered_Zsmooth15"
        dest         = '/data/pragallva/2023_repeat_ERA5/post_processing/filtered_parameters/%s/'%(FILTER_DIREC)  
        ...
        if not os.path.exists(file_check):
                ...
                latt_N    =  A_N['lat']
                lonn_N    =  A_N['lon']
                A_N       =  INTERP(A_N['field'])
                A_N = {'field':A_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( A_N, dest+'A_N.hdf5')

                F1_N      =  INTERP(h5saveload.load_dict_from_hdf5(route+'F1_N.hdf5')['field'])
                F1_N      = {'field':F1_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( F1_N, dest+'F1_N.hdf5')
                
                F2_N      =  INTERP(h5saveload.load_dict_from_hdf5(route+'F2_N.hdf5')['field'])
                F2_N      = {'field':F2_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( F2_N, dest+'F2_N.hdf5')

                F2a_N      =  INTERP(h5saveload.load_dict_from_hdf5(route+'F2a_N.hdf5')['field'])
                F2a_N      = {'field':F2a_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( F2a_N, dest+'F2a_N.hdf5')

                
                F3_N      =  INTERP(h5saveload.load_dict_from_hdf5(route+'F3_N.hdf5')['field'])
                F3_N      = {'field':F3_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( F3_N, dest+'F3_N.hdf5')

                # F_N       =  (F1_N + F2_N + F3_N)
                # Fa_N      =  (F1_N + F2a_N + F3_N)

                EPy1_N    =  INTERP(h5saveload.load_dict_from_hdf5(route+'EPy1_N.hdf5')['field'])
                EPy1_N    = {'field':EPy1_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( EPy1_N, dest+'EPy1_N.hdf5')
                
                EPy2_N    =  INTERP(h5saveload.load_dict_from_hdf5(route+'EPy2_N.hdf5')['field'])
                EPy2_N    = {'field':EPy2_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( EPy2_N, dest+'EPy2_N.hdf5')

                EPy1a_N    =  INTERP(h5saveload.load_dict_from_hdf5(route+'EPy1a_N.hdf5')['field'])
                EPy1a_N    = {'field':EPy1a_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( EPy1a_N, dest+'EPy1a_N.hdf5')
                
                EPy2a_N    =  INTERP(h5saveload.load_dict_from_hdf5(route+'EPy2a_N.hdf5')['field'])
                EPy2a_N    = {'field':EPy2a_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( EPy2a_N, dest+'EPy2a_N.hdf5')
                
                dEPz_dz_N =  INTERP(h5saveload.load_dict_from_hdf5(route+'dEPz_dz_N.hdf5')['field'])
                dEPz_dz_N = {'field':dEPz_dz_N, 'lat':latt_N, 'lon':lonn_N}
                h5saveload.save_dict_to_hdf5 ( dEPz_dz_N, dest+'dEPz_dz_N.hdf5')

                F_N       =  (F1_N['field'] + F2_N['field'] + F3_N['field'])
                Fa_N      =  (F1_N['field'] + F2a_N['field'] + F3_N['field'])

                load_files = False
                
                logging_object.write('-----------------------------------------------------------------------------------')
                logging_object.write('Zsmoothed but unfiltered data saved')
                logging_object.write('#####################################################################################')
                
                
        file_check = dest+'/EP_and_advective_fluxes.hdf5'
        load_EP = True
        if not os.path.exists(file_check):
                ...
                if load_files:
                        ...
                        logging_object.write('#####################################################################################')
                        logging_object.write('Loading each file individually..')
                        logging_object.write('-----------------------------------------------------------------------------------')
                    
                        F1_N      =  (h5saveload.load_dict_from_hdf5(dest+'F1_N.hdf5')['field'])
                        F2_N      =  (h5saveload.load_dict_from_hdf5(dest+'F2_N.hdf5')['field'])
                        F2a_N     =  (h5saveload.load_dict_from_hdf5(dest+'F2a_N.hdf5')['field'])
                        F3_N      =  (h5saveload.load_dict_from_hdf5(dest+'F3_N.hdf5')['field'])
                        F_N       =  (F1_N + F2_N + F3_N)
                        Fa_N      =  (F1_N + F2a_N + F3_N)
                        A_N       =  h5saveload.load_dict_from_hdf5(dest+'A_N.hdf5')
                        latt_N    =  A_N['lat']
                        lonn_N    =  A_N['lon']
                        A_N       =  (A_N['field'])
                        EPy1_N    =  (h5saveload.load_dict_from_hdf5(dest+'EPy1_N.hdf5')['field'])
                        EPy2_N    =  (h5saveload.load_dict_from_hdf5(dest+'EPy2_N.hdf5')['field'])
                        EPy1a_N   =  (h5saveload.load_dict_from_hdf5(dest+'EPy1a_N.hdf5')['field'])
                        EPy2a_N   =  (h5saveload.load_dict_from_hdf5(dest+'EPy2a_N.hdf5')['field'])
                        dEPz_dz_N =  (h5saveload.load_dict_from_hdf5(dest+'dEPz_dz_N.hdf5')['field'])
                ...
                del (EPa_and_advective_flux_convergence)
                del (heat_flux_term)
                del (Fx_gradient)
                del (Fax_gradient)
                del (F1x_gradient)
                ...
                del (F3x_gradient)
                del (dA_dt_N_daily)
                del (EMF_N_divergence)
                del (EMFa_N_divergence)
                del (residual)
                gc.collect()

                factorp =  +(np.cos(np.deg2rad((latt_N+dphi)[None,None,None,:,None])))
                factorm =  +(np.cos(np.deg2rad((latt_N-dphi)[None,None,None,:,None])))
                EPx     =  +(F3_N)
                EPyp    =  +(EPy2_N)/factorp
                EPym    =  +(EPy1_N)/factorm
                EPypa   =  +(EPy2a_N)/factorp
                EPyma   =  +(EPy1a_N)/factorm
                EPy     =  +(EPyp+EPym)/2
                EPya    =  +(EPypa+EPyma)/2
                EPz     =  +(dEPz_dz_N*(7e3)) #### Not sure about this. Why does it have 7000 factor? Value of H?

                EP_and_advective_fluxes = {'F_adv_x' :F1_N+F2_N,  'F1_adv_x':F1_N, \
                                           'F2_adv_x':F2_N,       'F_EP_x'  :F3_N, \
                                           'F_EP_y'  :EPy,        'F_EP_z'  :EPz,  \
                                           'lat':latt_N, 'lon':lonn_N, 'A_N':A_N}

                EPa_and_advective_fluxes = {'F_adv_x' :F1_N+F2a_N,  'F1_adv_x':F1_N, \
                                            'F2_adv_x':F2a_N,       'F_EP_x'  :F3_N, \
                                            'F_EP_y'  :EPya,        'F_EP_z'  :EPz,  \
                                            'lat':latt_N, 'lon':lonn_N, 'A_N':A_N}
 
                h5saveload.save_dict_to_hdf5 ( EPa_and_advective_fluxes, \
                                               dest+'EPa_and_advective_fluxes.hdf5')

                h5saveload.save_dict_to_hdf5 ( EP_and_advective_fluxes, \
                                               dest+'EP_and_advective_fluxes.hdf5')
...
#         file_check = dest+'/wave_activity_budget_integral.hdf5'
#         load_field_integral= True
#         if not os.path.exists(file_check):
            
#                 if load_EP:
#                     EP_and_advective_flux_convergence = h5saveload.load_dict_from_hdf5(dest+'/EP_and_advective_flux_covergence.hdf5')
...
#                         field_integral = h5saveload.load_dict_from_hdf5(dest+'/wave_activity_budget_integral.hdf5')

#                 for no_of_day_avg in range(1,25,1):
