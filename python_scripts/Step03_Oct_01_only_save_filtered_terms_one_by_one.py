#############################################################################################
#### This script calculates time filtering for >= 4 days ####
#### This script also does longitudinal filtering over 15 degrees in longitude (Approximately this is about the wavelength of a Rossby wave packet)
#### Can we justify it as Rossby wave length scale? - Check.
#############################################################################################
...
def time_filter_func(field, period_min=4, period_max=None, remove_seasonal_cycle=True, Zsmooth=True, seasonal_field=None):
    ...
    if np.isnan(field_reshape).any():
        ...
        seasonal_field, orig_shape = flatten_t(seasonal_field)
    ...
    filtered_data, freq_axis = tf.band_pass_filter_days(interped_data, seasonal_field = seasonal_field, infile=infile, logging_object=logging_object)
...
if __name__ == "__main__":
        ...
        if not os.path.exists(destination + input_dicti['MAIN_FILE_NAME']):
            ...
            latt_N       =  A_N['lat']
            lonn_N       =  A_N['lon']
            seasonal_A_N = h5saveload.load_dict_from_hdf5(input_dicti['master_destination']+'/seasonal/'+input_dicti['MAIN_FILE_NAME'])
            ...
            FILTERED_A_N =  time_filter_func(A_N['field'], \
                                             ...
                                             Zsmooth=['input_dicti'], seasonal_field = seasonal_A_N['field'])
