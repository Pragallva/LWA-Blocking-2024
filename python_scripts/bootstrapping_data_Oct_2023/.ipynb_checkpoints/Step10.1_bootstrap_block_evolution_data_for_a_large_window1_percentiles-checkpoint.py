...
def return_section_window(field, location = [-16,54],  date = '2010-Feb-4', block_days=16, lon_range=10, lat_range=10, time_range=10, time_range_plus=None, time_range_minus=None):
    ...
    ind_la[ind_la>=len(latt_N)] = len(latt_N)-1 #### This condition is to ensure that the values are not stored above the bounding box 20-80 N. All calculations in this paper are restricted to this bouunding box
    ind_la = list(ind_la)
...
def stats_over_many_blocks(FIELD, lon_range = 60, lat_range = 15, \
            ...
            if not os.path.exists(dest+name+'_stats_.hkl'):
                    ...
                    percentile_values = np.nanpercentile(BLOCK_window_list, percentiles, axis=0)
...
if __name__ == "__main__":
        ...
        # CHECK_DIR  =  source+'/'+lon_region+'/'+lat_region+'/BOOTSTRAPPING_BLOCKS/block_evolution_29_days/days4/synoptic_WA_fluxes/'
...
#             EP_and_advective_flux_convergence_synoptic = h5saveload.load_dict_from_hdf5(route_filtered+'EPa_and_advective_flux_covergence.hdf5')
            ...
            for blocking_days in range(4,20):
                        ...
                        if 'days%d'%(blocking_days) in blocking_details_day_wise:
                                    ...
                                    logging_object.write('Working on '+dest+'/synoptic_WA_budget/')

                                    ########## Save filtered wave activity budget terms #############
#                                     BLOCK_window_list_dxFfull        = stats_over_many_blocks(FIELD = (EP_and_advective_flux_convergence_synoptic['dF_dx']), \
...
#                                     BLOCK_window_list_dxF1full       = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dF1_adv_dx'], blocking_details=blocking_details,\
#                                                                                             dest=dest+'/synoptic_WA_budget/',name='BLOCK_window_list_dxF1_full_synoptic',\
#                                                                                             lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0, \
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

#                                     BLOCK_window_list_dxF2full       = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dF2_adv_dx'], blocking_details=blocking_details,\
#                                                                                             dest=dest+'/synoptic_WA_budget/',name='BLOCK_window_list_dxF2_full_synoptic',\
#                                                                                             lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0, \
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

#                                     BLOCK_window_list_dxF3full       = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dF_EPx_dx'],  blocking_details=blocking_details,\
#                                                                                             dest=dest+'/synoptic_WA_budget/',name='BLOCK_window_list_dxF3_full_synoptic',\
#                                                                                             lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0, \
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

#                                     BLOCK_window_list_dyEPyfull      = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dF_EPy_dy'], blocking_details=blocking_details,\
#                                                                                             dest=dest+'/synoptic_WA_budget/',name='BLOCK_window_list_dyEPy_full_synoptic',\
#                                                                                             lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

#                                     BLOCK_window_list_dzEPzfull      = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dF_EPz_dz'], blocking_details=blocking_details,\
#                                                                                             dest=dest+'/synoptic_WA_budget/',name='BLOCK_window_list_dzEPz_full_synoptic',\
#                                                                                             lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

#                                     BLOCK_window_list_dAdtfull       = stats_over_many_blocks(FIELD = EP_and_advective_flux_convergence_synoptic['dA_dt'], blocking_details=blocking_details,\
...
#                                                                                             save_bootstrap_raw=False, percentiles=PERCENTILES )[0]           
                                    ...
                                    logging_object.write('Working on '+dest+'/synoptic_WA_fluxes/')

                                    ##### Save unfiltered wave activity and wave activity fluxes ######
                                    BLOCK_window_list_Afull          = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['A_N'], \
                                                                                                    dest=dest+'/synoptic_WA_fluxes/', blocking_details=blocking_details,\
                                                                                                    name='BLOCK_window_list_A_full_synoptic',\
                                                                                                    lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
                                                                                                    save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

                                    BLOCK_window_list_Ffull          = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F_adv_x']+\
                                    ...
                                    BLOCK_window_list_F1full         = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F1_adv_x'], blocking_details=blocking_details,\
                                                                                            dest=dest+'/synoptic_WA_fluxes/',name='BLOCK_window_list_F1_full_synoptic',\
                                                                                            lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
                                                                                            save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

                                    BLOCK_window_list_F2full         = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F2_adv_x'], blocking_details=blocking_details,\
                                                                                            dest=dest+'/synoptic_WA_fluxes/',name='BLOCK_window_list_F2_full_synoptic',\
                                                                                            lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0, \
                                                                                            save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

                                    BLOCK_window_list_F3full         = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F_EP_x'], blocking_details=blocking_details,\
                                                                                            dest=dest+'/synoptic_WA_fluxes/', name='BLOCK_window_list_F3_full_synoptic',\
                                                                                            lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
                                                                                            save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

                                    BLOCK_window_list_F_EPy_full     = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F_EP_y'],   blocking_details=blocking_details,\
                                                                                            dest=dest+'/synoptic_WA_fluxes/', name='BLOCK_window_list_F_EPy_full_synoptic',\
                                                                                            lon_range = 60, lat_range = 15,  time_range_plus = Tp, time_range_minus = Tm, time_difference=0,\
                                                                                            save_bootstrap_raw=False, percentiles=PERCENTILES )[0]

                                    BLOCK_window_list_F_EPz_full     = stats_over_many_blocks(FIELD = EP_and_advective_fluxes_synoptic['F_EP_z'],   blocking_details=blocking_details,\
